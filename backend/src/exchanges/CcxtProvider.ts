  ...
  public async connect(config: ProviderConfig): Promise<void> {
    ...
    this.exchange.fetch = async (url: string, method = 'GET', headers: any = {}, body?: any) => {
      ...
      try {
        ...
        const errMsg = String(fetchError?.message || '').toLowerCase();
        if (this.exchangeId === 'binance' && (errMsg.includes('451') || errMsg.includes('restricted location') || errMsg.includes('eligibility'))) {
          console.warn(`[BINANCE 451 REGIONAL RESTRICTION BYPASS] ${method} ${url}`);
