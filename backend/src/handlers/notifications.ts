...
export async function sendPriceAlertNotification(
  ...
  if (env.FCM_PROJECT_ID && env.FCM_CLIENT_EMAIL && env.FCM_PRIVATE_KEY) {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`FCM v1 price alert send failed: ${response.status} ${errorText}`);
      } else {
        console.log(`FCM v1 price alert sent successfully to user ${userId}`);
      }
    } catch (err) {
      console.error("FCM v1 price alert failed:", err);
    }
    return;
  }
  ...
  if (env.FCM_SERVER_KEY) {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`FCM Legacy price alert send failed: ${response.status} ${errorText}`);
      }
    } catch (error) {
      console.error("FCM Legacy price alert notification failed:", error);
    }
    return;
  }
